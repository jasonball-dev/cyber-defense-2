...
public class Pathfinding : MonoBehaviour
{
    ...
    public Pathfinding(int width, int height, Vector3 start) {
        grid = new Grid<PathNode>(width, height, 10f, start, (Grid<PathNode> g, int x, int z) => new PathNode(g, x, z));
    }
    ...
    public List<Vector3> FindPath(Vector3 startWorldPosition, Vector3 endWorldPosition) {
        ...
        if (path == null) {
            ...
            foreach (PathNode pathNode in path) {
                vectorPath.Add(new Vector3(pathNode.x, 0, pathNode.z) * grid.GetCellSize() + Vector3.one * grid.GetCellSize() * .5f + new Vector3(-70, 0, -70));
            }
            return vectorPath;
        }
    }
